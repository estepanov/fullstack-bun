...
export class ChatPresenceService {
  ...
  async addPresence(presenceId: string, role: ClientRole): Promise<void> {
    try {
      ...
      appLogger.error({ error, presenceId, role }, "Failed to add presence:");
    }
  }
  ...
  async removePresence(presenceId: string, role: ClientRole): Promise<void> {
    try {
      ...
      appLogger.error({ error, presenceId, role }, "Failed to remove presence:");
    }
  }
  ...
  async touchPresence(presenceId: string, role: ClientRole): Promise<void> {
    try {
      ...
      appLogger.error({ error, presenceId, role }, "Failed to touch presence:");
    }
  }
  ...
  async clearAll(): Promise<void> {
    try {
      const roles: ClientRole[] = ["guest", "member", "admin"];
      await Promise.all(roles.map((role) => redis.del(this.getPresenceKey(role))));
